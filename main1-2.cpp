...
int main() {
	...
	while (menu != 0) {
		...
		if (menu == 7) std::cout << "Focal distance is:" << el.getf() << std::endl;
		...
		if (menu == 13) {
			...
			if (isnan(y1)&&isnan(y2)) {
				std::cout << "There are no such points on ellipse" << std::endl;
			}
			else {
				if (!isnan(y1)) std::cout << std::endl << "y(" << a << ")is:" << y1 << std::endl;
				if (!isnan(y2)&&y1!=y2) std::cout << "y(" << a << ")is:" << y2 << std::endl;
			}
		}
	}
	return 0;
